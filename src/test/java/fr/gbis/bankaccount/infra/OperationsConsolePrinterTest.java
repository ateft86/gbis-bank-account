...
import fr.gbis.bankaccount.operation.CheckOperations;
...
class OperationsConsolePrinterTest {
	...
	void should_print_operations() {
		...
		var account = new Account(new IBAN("FR", "12", "34567890"), "My account", Currency.getInstance("EUR"),
				Set.of(new Operation(DEPOSIT, new Amount(new BigDecimal(500), EUR), "Salary",
						Instant.parse("2020-01-10T12:34:56.00Z")),
						new Operation(WITHDRAWAL, new Amount(new BigDecimal(100), EUR), "Rent",
								Instant.parse("2020-01-11T10:10:10.00Z")),
						new Operation(WITHDRAWAL, new Amount(new BigDecimal(75), EUR), "Shopping",
								Instant.parse("2020-01-12T11:11:11.00Z"))));
