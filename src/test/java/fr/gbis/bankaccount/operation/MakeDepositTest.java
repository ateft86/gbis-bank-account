...
class MakeDepositTest {
	...
	void should_make_a_deposit_of_ten_euros_when_deposit_five_dollars() {
		...
		// When
		var accountWithNewOperation = makeDeposit.make(account, new Amount(new BigDecimal(5), USD), "A deposit");

		// Then
		assertThat(accountWithNewOperation.operations).hasSize(2)
				.contains(new Operation(DEPOSIT, new Amount(new BigDecimal(10), EUR), "A deposit", Instant.now(clock)));
	}
