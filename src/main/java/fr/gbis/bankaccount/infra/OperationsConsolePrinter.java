...
public final class OperationsConsolePrinter {
	...
	public OperationsConsolePrinter(CheckOperations checkOperations) {
		...
		this.dateTimeFormatter = DateTimeFormatter.ofLocalizedDateTime(SHORT).withLocale(FRANCE).withZone(UTC);
	}
	...
	public void printOperations(Account account) {
		...
		checkOperations.seeHistory(account)
				.forEach(history -> System.out.println(String.format(PRINTING_PATTERN, history.operation.operationType,
						dateTimeFormatter.format(history.operation.date), formatAmount(history.operation.amount),
